...
class CustomDataset(BaseDataset):
    ...
    def load(self) -> dict[str, list[CameraProperties] | None]:
        ...
        for subset in self.subsets:
            ...
            data = camera_info["ptr_wrapper"]["data"]
            # 兼容 value0（EgoNeRF/Ricoh360）或 OpenMVG 导出的其他键名
            if "value0" in data:
                intrinsic = data["value0"]
            elif isinstance(data, dict) and "width" in data and "height" in data:
                intrinsic = data
                ...
                for v in data.values():
                    if isinstance(v, dict) and "width" in v and "height" in v:
                        intrinsic = v
                        break
                if intrinsic is None and "width" in data and "height" in data:
                    intrinsic = data
                else:
                    intrinsic = intrinsic or data
            else:
                intrinsic = data
            if "width" not in intrinsic or "height" not in intrinsic:
                raise Framework.DatasetError(
                    f'openMVG intrinsics 中缺少 width/height，请检查 openMVG/data_openmvg_{subset}.json 的 intrinsics 结构。'
                )
            original_width = int(intrinsic["width"])
            original_height = int(intrinsic["height"])
            original_focal = original_width / (2 * math.pi * math.cos(self.FOCAL_ANGLE / 180 * math.pi))
            raw_extrinsics = openmvg_data["extrinsics"]
            # 兼容 list（convert_openmvg_to_ricoh360_json 输出）与 dict（官方 Ricoh360）
            if isinstance(raw_extrinsics, list):
                extrinsics_dict = {e.get("key", i): e for i, e in enumerate(raw_extrinsics)}
            else:
                extrinsics_dict = raw_extrinsics
            images = []
            for view in openmvg_data["views"]:
                info = view["value"]
                pose_id = info["ptr_wrapper"]["data"]["id_pose"]
                # JSON 键可能是 int 或 str，兼容两种
                ext_obj = extrinsics_dict.get(pose_id) or extrinsics_dict.get(str(pose_id))
                if ext_obj is None:
                    raise Framework.DatasetError(
                        f'openMVG 中 view 的 id_pose={pose_id} 在 extrinsics 中不存在（键多为 int/str 不一致）。'
                    )
                extrinsics = ext_obj["value"]
                c2w = torch.eye(4)
